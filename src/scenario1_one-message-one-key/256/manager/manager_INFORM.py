...
#adapted and extended for scenario 1 ("one message – one key") with kms integration, inform aes-256 variant

"""
snmpv3 client (authPriv: SHA + AES-256) with kms integration — INFORM variant

policy (scenario 1: one message – one key):
  1) fetch enc_key (256-bit) from master kms  -> (key_ID, key_bytes)
  ...
  4) send exactly one snmp INFORM (sha + aes-256)
  5) repeat

runs indefinitely until interrupted (Ctrl+C). requires aes-256 (priv) support on both sides.
"""
...
logging.info("snmp privacy on manager: AES-256")
...
def _derive_aes256(key_bytes: bytes) -> bytes:
    #return exactly 32B (if not 32B, use sha256 digest which is 32B)
...
def get_enc_key_from_kms(master_base: str, app_id: str, n_keys: int = 1, size_bits: int = 256) -> Tuple[Optional[bytes], Optional[str], Optional[str]]:
...
def run_snmp_inform(agent_host: str, agent_port: int, user: str, auth_pass: str, priv_key_bytes: bytes) -> bool:
    #send a single snmpv3 INFORM (coldStart) with authPriv (sha + aes-256)
    logging.info("step 3 – SNMP INFORM (AES-256) — 1 message = 1 key")

    authData = UsmUserData(
        userName=user,
        authKey=auth_pass, authProtocol=usmHMACSHAAuthProtocol,
        privKey=_derive_aes256(priv_key_bytes), privProtocol=usmAesCfb256Protocol
    )
    ...
    try:
        errIndication, errStatus, errIndex, varBinds = next(sendNotification(
            SnmpEngine(), authData, transport, ctx, 'inform', ntf
        ))
        if errIndication:
            logging.warning("inform ei: %s", errIndication)
            return False
        if errStatus:
            logging.warning("inform es: %s", errStatus.prettyPrint())
            return False
...
def main():
    ...
    while True:
        ...
        if not wait_key_applied(AGENT_HOST, key_id, max_wait_s=6.0):
            used_ids.add(key_id); last_key_id = key_id
            time.sleep(1); continue
