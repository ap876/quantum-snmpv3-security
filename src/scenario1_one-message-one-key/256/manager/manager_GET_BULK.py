...
"""
snmpv3 client (authPriv: SHA + AES-256) with kms integration, GETBULK mode
...
BULK_MAX_REPETITIONS = 10             #number of rows to fetch in one bulk
...
#POLICY: 1 key = 1 message
#wait so the agent applies the key
...
def post_key_id_to_agent(url: str, user: str, key_id: str, timeout_sec: int = 5) -> bool:
    #inform the agent which key_ID to pull from slave kms
...
def run_snmp_getbulk(agent_host: str,
    ...
    #usm user with priv key derived from kms (aes-256)
    config.addV3User(
        snmpEngine, user,
    ...
    #contextEngineId=None, contextName='' before N/M
...
def main():
    #dedup tracking to avoid accidental key reuse
    ...
    try:
        while True:
            ...
            #give agent time to rotate usm key
