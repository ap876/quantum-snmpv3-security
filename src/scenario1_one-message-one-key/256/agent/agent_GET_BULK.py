...
# adapted and extended for scenario 1 ("one message – one key") with KMS integration, get-bulk aes-256 variant
...
def rotate_usm_keys(user: str, new_priv_key_bytes: bytes, also_rotate_auth: bool = False):
    #rotate usm keys for a specific user; by default only priv (aes-256)
    try:
...
def fetch_dec_key_from_slave(slave_base: str, app_id: str, key_id: str, timeout=DEC_TIMEOUT_SEC):
    ...
    returns:
      ("ok", key_bytes) | ("gone", None) | ("error", None)
    """
...
def process_key_id_async(user: str, key_id: str):
    #background worker -> fetch dec_key and rotate usm; ensures idempotency
