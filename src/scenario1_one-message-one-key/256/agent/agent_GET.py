...
- for each snmp message, the manager fetches a fresh enc_key and posts key_ID to the agent
- the agent, on /kms, fetches the corresponding dec_key from slave kms and rotates the usm key for the given user
- duplicate key_ID notifications are treated idempotently (already processed/in-flight)
...
#initial usm users for compatibility/demo
...
#primary aes-256 user (priv rotated via /kms); placeholder 32B
