...
"""
snmpv3 client (authPriv: SHA + AES-128) with kms integration — TRAP variant
...
policy (scenario 1: one message – one key):
  ...
  4) send exactly one snmp TRAP (coldStart) using that fresh key
  5) repeat

note: TRAP is unacknowledged (no response), so we only log local errors.
"""
...
def _derive_aes128(key_bytes: bytes) -> bytes:
    #return exactly 16B
    #if not 16B, derive via sha256 and take first 16B
    if len(key_bytes) == 16:
        return key_bytes
...
def run_snmp_trap(agent_host: str, agent_port: int, user: str, auth_pass: str, priv_key_bytes: bytes) -> bool:
    #send one snmpv3 TRAP (coldStart) with authPriv (sha + aes-128)
    #TRAP has no ack
    logging.info("step 3 – SNMP TRAP — 1 message = 1 key")
    ...
    #standard trap ->coldStart (HLAPI adds sysUpTime.0 & snmpTrapOID.0 automatically)
    ntf = NotificationType(ObjectIdentity('1.3.6.1.6.3.1.1.5.1'))

    try:
        errIndication, errStatus, errIndex, varBinds = next(sendNotification(
            SnmpEngine(), authData, transport, ctx, 'trap', ntf
        ))
        if errIndication:
            logging.warning("trap ei: %s", errIndication)
            return False
        if errStatus:
            logging.warning("trap es: %s", errStatus.prettyPrint())
            return False
        logging.info("trap sent (no ack expected).")
        return True
