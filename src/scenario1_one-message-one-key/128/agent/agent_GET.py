...
"""
snmpv3 agent (usm) with kms integration, idempotent dec_key retrieval:
...
- on successful dec_key fetch (200 ok), rotates usm priv key (aes-128)
...
DEC_TIMEOUT_SEC   = 3
DEC_RETRIES       = 6
DEC_RETRY_DELAY_S = 0.7  #total retry window ~4s

#deduplication of key_ID processing
in_flight = set()      #key_ID currently being processed
processed_ids = set()  #key_ID successfully processed or terminally closed
...
def rotate_usm_keys(user: str, new_priv_key_bytes: bytes, also_rotate_auth: bool = False) -> None:
    try:
...
def fetch_dec_key_from_slave(slave_base: str, app_id: str, key_id: str, timeout: int = DEC_TIMEOUT_SEC) -> Tuple[str, bytes | None]:
    """
    ...
    returns:
      ("ok",   key_bytes)  -> key delivered
      ("gone", None)       -> key not found / already consumed (404/410 or explicit message)
      ("error", None)      -> other errors (transport/json/decode)
    """
    ...
    if status == 200:
        try:
            #try raw read then optional gzip
        ...
        if "gzip" in enc and raw_bytes[:2] == b"\x1f\x8b":
            try:
                ...
                logging.warning("dec_keys: gzip decompress failed: %s; trying plain json", e)
    ...
    #non-200 -> try to read short body for hints
    text = ""
    try:
        try:
            r.raw.decode_content = False
...
def process_key_id_async(user: str, key_id: str) -> None:
...
class QKDHandler(BaseHTTPRequestHandler):
    def log_message(self, fmt, *args):
        # keep http server quiet in stdout
        return
    ...
    def do_POST(self):
        ...
        #main entry point -> manager notifies we should fetch dec_key for key_ID
        if path == "/kms":
            user   = data.get("user") or QKD_USER_DEFAULT
