...
#adapted and extended for scenario 2 ("time-based rekey") with KMS integration
...
loop<:
  1) if it’s time to rekey:
      - fetch enc_key from master kms  → (key_ID, key_bytes)
      - post key_ID to agent /kms      → agent pulls dec_key from slave kms and rotates usm
      - wait briefly so the agent can apply the key
  2) send an snmp GET using the CURRENT key (reused until next rekey)
  3) sleep and repeat

if kms has no material at rekey time:
  --insufficient-policy chooses behavior:
    * reuse: keep using the previous key until the next attempt
    * wait:  actively retry up to --wait-timeout (sleeping --wait-retry); if still none, fall back to reuse
    * abort: skip sending in this cycle until a fresh key is available
"""
...
def _trim_to_json(raw: str) -> str:
    #strip any banner/noise before the first '{'
    i = raw.find("{")
...
def post_key_id_to_agent(url: str, user: str, key_id: str, timeout_sec: int = 5) -> bool:
    #tell the agent which key_ID to fetch from slave kms
...
def run_snmp_get(agent_host: str, agent_port: int, user: str, auth_pass: str, priv_key_bytes: bytes, oid_str: str):
    #send one snmp GET using the current key (aes-128)
    logging.info("step 3 – SNMP GET %s (time-based rekey, AES-128)", oid_str)
    snmpEngine = engine.SnmpEngine()
    config.addTransport(snmpEngine, udp.domainName, udp.UdpTransport().openClientMode())

    #register usm user (auth: sha string, priv: aes-128 bytes)
    config.addV3User(
        snmpEngine, user,
...
#time-based rekey manager (client side)
class TimeRekeyClient:
    def __init__(self, *,
        ...
        self.apply_sleep_s = apply_sleep_s  # small pause so the agent can apply the key
        ...
        #recent key_ID dedupe (avoid accidental reuse if kms re-serves same id)
        self.recent_ids = deque(maxlen=2000)
        self.recent_set = set()

    def _try_fetch_and_apply(self):
        #attempt one rekey cycle: fetch enc_key → notify agent → wait briefly → activate locally
        key_bytes, _key_b64, key_id = get_enc_key_from_kms(self.master_kms_base, self.master_app_id)
        if not key_id:
        ...
        #give the agent a moment to apply
        ...
        logging.info("rekey ok: key_ID=%s (valid until %.0f)", key_id, self.next_rekey_at)
        return True, ""

    def ensure_key(self):
        #ensure we have a valid key for this cycle -> apply policy if kms lacks bits
        #returns (can_send, reason, fresh_key_flag)
        now = time.time()

        #no key yet or interval expired → attempt rekey
        if (self.active_key_bytes is None) or (now >= self.next_rekey_at):
            ok, reason = self._try_fetch_and_apply()
            if ok:
                return True, "fresh", True

            #kms didn’t provide a key → apply insufficient policy
            if self.insufficient_policy == "wait":
                ...
                if self.active_key_bytes is not None:
                    logging.warning("wait timeout — continuing with the previous key (reuse).")
                    return True, "reuse_after_wait_timeout", False
                else:
                    logging.error("wait timeout — no key available (abort).")
                    return False, "no_key_after_wait", False

            if self.insufficient_policy == "reuse":
                if self.active_key_bytes is not None:
                    logging.warning("rekey failed (%s) — reusing previous key.", reason)
                    return True, "reuse", False
                else:
                    logging.error("rekey failed (%s) — no previous key to reuse (abort).", reason)
        ...
        #still within interval → keep using current key
...
def build_args():
    ...
    #time-based rekey
    ap.add_argument("--rekey-interval", type=int, default=30)  # change to 60 if you prefer
    ...
    #send rate
    ap.add_argument("--pause", type=float, default=1.0, help="sleep between GET messages (s)")
...
def main():
    ...
    #instantiate the time-based rekey controller
    ...
    try:
        while True:
            ...
            if not can_send:
                #policy=abort (or no key yet): skip this cycle
                time.sleep(args.pause)
                continue

            #send one GET with the current key
            run_snmp_get(AGENT_HOST, AGENT_PORT, USERNAME, AUTH_PASS, rekey.active_key_bytes, OID_STR)
            sent_count += 1
            logging.info("GET sent (fresh_key=%s, key_ID=%s). total=%d",
